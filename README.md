...
- 事件总线：Worker 退出时，无论成功失败，都向消息队列推一条 task-notification；Agent Loop 收到后读取 summary/result_manifest，再生成对你的最终回复。这个模式与你说的三步完全一致，而且和 Claude Code 的 hook/notification、长任务提醒、持久线程、定时任务方向是一致的。
- 监控层：Cron 不负责跑业务，只负责巡检；Watchdog 定时检查心跳、进度时间、退出码、失败率，发现“未退出但卡死”时推 task-alert。
- 安全层：浏览器与执行环境尽量放在隔离容器/VM，限制域名、动作白名单；购买、删除、登录等高影响动作必须 HITL。
- 扩展层：所有外部系统优先经 MCP/连接器接入，这样你以后加 Figma、文档、网页、内部服务时，不需要改 Agent 核心。
